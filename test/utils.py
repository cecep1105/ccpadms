...
class MailThread(threading.Thread):
	def __init__(self):
		...
		info=open("%s"%os.path.split(__file__)[0]+"/eMailCfg.txt").read()
...
def getJSResponse(content=None):
	...
	if content:
		try:
			content=str(content).encode(settings.NATIVE_ENCODE)
		except:
