...
from iclock.dataproc import getFW
from django.conf import settings
from iclock.dataproc import appendDevCmd, devUpdateFirmware
from .mylib.iclockutils import writeattdata
from .mylib.writedata import parseLogDataInFile, run_writedata, performPostDataFile
from .tasks import process_attdata, process_attdata2
from .utils import *
...
def lineToUser(cursor, device, line):
		...
		#f = open('tmp\DevPostUser%s.txt'%dt,'a')
		#f.write(line + '\n')
		#f.close()
		if ops[0] == "USER":
			try:
...
def checkDevice(request, response):
	# print('checkDevice') and settings.DEBUG
	# print(cache.get('iclock_AEYU182060581'))
	pushver = 0.0
	if request.GET.get('pushver'):
		try:pushver = float(request.GET["pushver"])
		except:pass
	...
	# print('checkDevice: SN=%s' % sn) and settings.DEBUG
	try:
		device = getDevice(sn)
		if device and float(device.PushVersion) != pushver:
			device.PushVersion = pushver
			device.save()
		
		if device and device.DelTag:
			if settings.ICLOCK_AUTO_REG:
				device.DelTag = None
				device.save()
			else:
				return None
		if device and device.State == DEV_STATUS_PAUSE:
			return None
	except Exception as e:
		print(e)
		traceback.print_exc()
		if len(sn) >= 6 and settings.ICLOCK_AUTO_REG and not request.GET.get('PIN'): #若SN正确，设置可以自动注册，且没有查询用户信息的话，注册该考勤机
			try:
				newdevice = RegisteredDevice.objects.get(SN=sn)
				newdevice.LastActivity = datetime.datetime.now()
				mirror_ip = request.GET.get('ORIGINIP',None)
				# print('mirror_ip',mirror_ip)
				if mirror_ip is None:
					newdevice.IPAddress = request.META["REMOTE_ADDR"]
				else:
					newdevice.IPAddress = mirror_ip
				newdevice.save()
				from iclock.zktools.zk import ZK
				mac = ''
				try:
					zk = ZK(newdevice.IPAddress)
					zkconn = zk.connect()
					mac = zkconn.get_mac().upper()
				except:
					pass
				# print('checkDevice: update la+ip') and settings.DEBUG
				wsinfo('iclock', 'device_register', {'SN': newdevice.SN, 'ip': newdevice.IPAddress, 'mac': mac,
				...
				print('line307',e)
				newdevice = RegisteredDevice(SN=sn, Function='0',
								LastActivity=datetime.datetime.now(), IPAddress=request.META["REMOTE_ADDR"],
								DeptID=getDefaultDept())
				try:
					newdevice.save()
					# print('checkDevice: regdevice created with function=0') and settings.DEBUG
				except Exception as e:
					from common.zk import ZK
					mac=''
					try:
						zk = ZK(newdevice.IPAddress)
						zkconn = zk.connect()
					...
					# print('checkDevice: regdevice already exist') and settings.DEBUG
					wsinfo('adms', 'device_register', {'SN': newdevice.SN, 'ip': newdevice.IPAddress, 'mac': mac, 'time': datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
					print(e)

				return None

			if newdevice.Function == "0" or newdevice.DeptID==0 or newdevice.DeviceName == '':
				return None

			device = iclock(
				SN=sn,
				Alias=request.META["REMOTE_ADDR"],
				LastActivity=datetime.datetime.now(),
				IPAddress=newdevice.IPAddress,
				DeptID=newdevice.DeptID,
				MAC=newdevice.MAC,
				State=True,
				Function=newdevice.Function,
	...
	# print('checkDevice: result: %s' % device) and settings.DEBUG
	return device
...
def postDataToFile_BySuper(request, device, rawData):
	...
	LogStamp = ""
	dataType = "OPLOG"
	if request.GET.get('Stamp'): #and request.GET.get('table'):
		if request.GET.get('table'): #新协议
			LogStamp = request.GET['Stamp']
			if request.GET['table'] == 'ATTLOG':#by super 2010-07-22 上传考勤记录
				dataType = "TRANSACTIONS"
			elif request.GET['table'] == 'OPERLOG':#by super 2010-07-22 上传用户信息和系统日志
				rawData = decode_data(rawData)
	elif request.GET.get('OpStamp'):
		rawData = decode_data(rawData)
	else:
		return resp
	...
	s_data = ":%s: SN=%s\tIP=%s\tTIME=%s\n%s\n\n" % (dataType, device.SN,
			...
			rawData)
	if (request.GET.get('table') and request.GET['table'] == 'OPERLOG') or request.GET.get('OpStamp'):
	...
	try:
		...
		device.LogStamp = request.GET['Stamp']
		device.OpLogStamp = request.GET['OpStamp']
		resp = "OK:%d\n" % 1
	...
	try:
		...
		if request.GET.get('action'):
			...
			if request.GET.get('PIN'):
				save = request.GET.get('save') and (request.GET['save'] in ['1', 'Y', 'y', 'yes', 'YES']) or False
				try:
					pin = request.GET['PIN']
				...
				pushverValue = None
				if request.GET.get('pushver'):
					pushverValue = request.GET['pushver']				
				attlogStamp = "Stamp"
			...
			if request.GET.get('Stamp'): #and request.GET.get('table'):
				if request.GET.get('table'): #新协议				
					if request.GET['table'] == 'ATTLOG':#by super 2010-07-22 上传考勤记录
						...
						try:
							...
							process_attdata2.delay(old_file,raw_Data.decode())
							# process_attdata2(old_file,raw_Data.decode())
						except Exception as e:
							print(e)
							appendFile(e)
							resp += u"ERROR:%s\n" % e
						...
						try:
							...
							device.OpLogStamp = request.GET['Stamp']
							resp += "OK:%d\n" % 1
					elif request.GET['table'] == 'ATTPHOTO':#by super 2010-07-24 上传照片
					...
					try:
						write_to_file(old_file, s_data.encode("utf-8"))
						writeattdata(device, rawData.decode())
					except Exception as e:
						appendFile(e)
						resp += u"ERROR:%s\n" % e
						print(u"ERROR:%s\n" % e)
				...
				try:
					...
					device.OpLogStamp = request.GET['OpStamp']
					resp += "OK:%d\n" % 1
			elif request.GET.get('FPImage'):#/iclock/cdata?SN=7160048&FPImage=/tmp/enroll_fp_1.bmp&PIN=968&FID=0
				pin = request.GET["PIN"]
				try:
					fid = request.GET["FID"]
				except: fid = 0
				fName = request.GET['FPImage']
...
#			open("c:/postdata.txt", "a+").write("POST FROM: %s(%s)%s----------\n"%(request.META["REMOTE_ADDR"],request.META["PATH_INFO"],datetime.datetime.now()))
#			open("c:/postdata_%s.txt"%(ccc), "a+").write(request.body)
			raw_Data = request.body
			if settings.ENCRYPT:
				import lzo
				rawData = lzo.bufferDecrypt(raw_Data, device.SN)
...
def getreq(request):
	...
	try:
		...
		# wsinfo('iclock', 'device_request', device.SN)
		...
		c = 0
		wsinfo('iclock', 'device_info', { 'sn': device.SN, 'la': device.LastActivity.strftime("%Y-%m-%d %H:%M:%S"), 'devinfo': info })
		...
		if len(cmds) > 0:
			...
			for acmd in cmds:
				if acmd.CmdTransTime == None or ((datetime.datetime.now() - acmd.CmdTransTime).seconds >=0):
					if acmd.CmdContent.find("DATA USER ") == 0 or acmd.CmdContent.find("DATA UPDATE USERINFO") == 0 or acmd.CmdContent.find("SMS ") >= 0 or acmd.CmdContent.find("WEATHER ") == 0:
						cc = acmd.CmdContent
					else:
					...
					if acmd.CmdContent in ["REBOOT", "RESTART"]: break;
		if c == 0:
			resp += "OK"
		checkAndSave(device)
	except  Exception as e:
...
def postPhoto(request):
	...
	try:
		# pin = request.GET["PIN"]
		# pin = request.body[:74].decode()
		pin = request.body.split("CMD=uploadphoto".encode())[0].decode()
		...
		f = open(fname, "wb")
		d = request.body
		if "CMD=uploadphoto".encode() in d: d = d.split("CMD=uploadphoto".encode())[1][1:]
		if "CMD=realupload".encode() in d: d = d.split("CMD=realupload".encode())[1][1:]
		f.write(d)
		f.close()
		#if request.GET.get('PhotoStamp'):
		if request.GET.get('Stamp') and request.GET.get('table') and request.GET['table'] == 'ATTPHOTO':#by super 2010-07-22 修改参数
			devlog(SN=device, Cnt=1, OP=u"PICTURE", Object=pin, OpTime=datetime.datetime.now()).save()
			#device.PhotoStamp=request.GET['PhotoStamp']
			device.PhotoStamp = request.GET['Stamp']
...
def write_to_file(file_name, s_data):
	...
	f.write(str(s_data.decode('utf-8')))
	#f.flush()
	f.close()
